...
namespace latijns
{
	...
	{
	public:
		struct ptr : public std::shared_ptr<json>
		{
			ptr() {}
			ptr(json* address) : std::shared_ptr<json>(address) {}
			ptr(std::nullptr_t& other) : std::shared_ptr<json>(other) {}
			ptr(const std::shared_ptr<json>& other) : std::shared_ptr<json>(other) {}
			ptr(const std::shared_ptr<json>&& other) : std::shared_ptr<json>(other) {}

			const json::ptr operator[](const char* a_key) const;
			const json::ptr operator[](int a_index) const;
		};
		...
		void set_data(const char* a_value, size_t a_length = 0, std::shared_ptr<char> a_owner = nullptr);
