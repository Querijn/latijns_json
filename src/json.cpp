...
namespace latijns
{
	...
	namespace json_parser
	{
		...
		inline void parse_value(std::shared_ptr<parser_data>& a_parser, const char* a_data, size_t& i, size_t a_length, json::type* a_type)
		{
			...
			case '9': case '-':
			{
				...
				if (a_parser->location == parser_location::in_array)
				{
					// TODO: Remove this when we confirmed it's correct now
					assert(
						a_data[start - 1] != '0' &&
						a_data[start - 1] != '1' &&
						a_data[start - 1] != '2' &&
						...
						a_data[start - 1] != '9' &&
						a_data[start - 1] != '-');
					a_parser->children[a_parser->children_count++] = std::make_shared<json>(a_data + start, end + 1, a_parser->data);
				}
	...
	{
		set_data(a_data, a_length, a_owner);
	}

	void json::set_data(const char* a_data, size_t a_length, std::shared_ptr<char> a_owner)
	{
	...
	const json::ptr json::ptr::operator[](const char* a_key) const
	{
		return get()->operator[](a_key);
	}
	
	const json::ptr json::ptr::operator[](int a_index) const
	{
		return get()->operator[](a_index);
	}
}
